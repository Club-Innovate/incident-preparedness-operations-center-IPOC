...
| Minimum Necessary / Data Minimization | Yes | Partial | User DTOs intentionally omit sensitive fields, redaction helpers for exports + auto-generated prioritized review ledger (`security-compliance/controls/endpoint-minimum-necessary-reviews.csv`) from audit matrix (`audit/tmp/_tmp_endpoint_matrix_generated.md`) | Execute backlog closure on high-risk rows and attach endpoint-level evidence |
